...
from models import AnalysisRequest, RiskReport, DocumentMeta
...
async def analyze_document(file: UploadFile = File(...)):
    ...
    # 1. Parse and section the document
    ...
    # 2. RAG: retrieve relevant compliance guidelines per section
    guideline_contexts = {}
    for section in sections:
        guideline_contexts[section["id"]] = rag_engine.retrieve(section["text"], top_k=3)

    # 3. Rule engine: deterministic hard checks
    rule_findings = rule_engine.analyze(sections)

    # 4. LLM analysis: semantic risk detection + suggestions
    llm_findings = llm_analyzer.analyze_sections(sections, guideline_contexts)

    # 5. Merge findings and build report
...
async def get_risk_report(report_id: str):
    """Retrieve a previously generated risk report by ID."""
...
async def list_reports():
    """List all analyzed documents and their risk summaries."""
    return [
        DocumentMeta(
...
def _build_report(report_id, filename, sections, rule_findings, llm_findings) -> RiskReport:
    """Merge rule engine + LLM findings into a unified report."""
    all_findings = rule_findings + llm_findings

    # Deduplicate by (section_id, issue_type) roughly
    seen = set()
    unique_findings = []
